...
return function()
    ...
    assert(config:getDownloadImages() == true)
    assert(config:getArchiveFinished() == true)
    config:setArchiveFinished(false)
    assert(config:getArchiveFinished() == false)
    config:setArchiveFinished(true)
    assert(config:getArchiveFinished() == true)
    assert(config:getPropagateHighlightDeletions() == false)
    config:setPropagateHighlightDeletions(true)
    assert(config:getPropagateHighlightDeletions() == true)
    config:setPropagateHighlightDeletions(false)
    assert(config:getPropagateHighlightDeletions() == false)
    assert(config:getMaxImageBytes() == 2 * 1024 * 1024)
    assert(config:getMaxArticleImageBytes() == 8 * 1024 * 1024)
    assert(config:getMaxImagesPerArticle() == 20)
    assert(config:getMaxRawSourceBytes() == 64 * 1024 * 1024)
    assert(config:getMinRawSourceFreeBytes() == 128 * 1024 * 1024)
    assert(config:isSyncCategoryEnabled("pdf") == false)
    assert(config:isSyncCategoryEnabled("epub") == false)
    config:setSyncCategoryEnabled("pdf", true)
    config:setSyncCategoryEnabled("epub", true)
    assert(config:isSyncCategoryEnabled("pdf") == true)
    assert(config:isSyncCategoryEnabled("epub") == true)
    config:setDownloadImages(false)
    assert(config:getDownloadImages() == false)
    config:setDownloadImages(true)
    assert(config:getDownloadImages() == true)
