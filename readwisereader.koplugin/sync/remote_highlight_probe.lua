...
function Probe:run(local_path)
    ...
    if document.is_local_present ~= true or not self.file_exists(document.local_path) then
        return nil, domainError("local_missing", "The managed Reader file is missing.")
    end
    if document.local_format ~= "epub" and document.local_format ~= "html" then
        return nil, domainError(
            "format",
            "Gate 17A only probes rolling EPUB/HTML documents. PDF import needs a separate position spike."
        )
    end
