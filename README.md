...
**v1.1.0 is accepted on the target PW3.** It adds historical Reader → KOReader highlight/note import for the currently-open managed rolling EPUB/HTML. It reconciles those remote highlights before outbound annotation creates, uses a durable historical/incremental cache, suppresses unsafe collisions instead of creating duplicates, and preserves notes through KOReader's native sidecar path.

**v1.2.0 is accepted on the target PW3.** It extends that historical Reader → KOReader path to the currently-open managed original PDF using KOReader's native paging geometry. Normal Sync reconciles Reader PDF highlights before outbound creates, writes imported annotations to the KOReader sidecar without modifying PDF bytes, preserves notes, and durably deduplicates against the existing Reader child IDs. The consolidated acceptance imported the remaining PDF highlight, preserved both across close/reopen, and an unchanged second Sync created no local or remote duplicate.
